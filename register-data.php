<?php
    ...
    for($i=0; $i<$count; $i++){
        $sql = "INSERT INTO esperienze (argomenti, annofrequenza, votazione, idutente, identita) VALUES (?, ?, ?, ?, ?)";
        $smt = $DB->save($sql);
        $smt -> execute([$argomenti[$i], $anno[$i], $votazione[$i], $idutente, $identita[$i]]);
    }
